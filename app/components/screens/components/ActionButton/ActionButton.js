...
export default class ActionButton extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.resetToken !== this.state.resetToken) {
      if (nextProps.active === false && this.state.active === true) {
        ...
        Animated.timing(this.state.anim, { toValue: 0, duration: 300, easing: 'Ea' }).start();
        setTimeout(() => this.setState({ active: false, resetToken: nextProps.resetToken }), 350);
        return;
      }
  ...
  _renderActions() {
    ...
    const actionStyle = {
      ...
      paddingTop: 0,
      zIndex: this.props.zIndex,
      marginBottom: this.props.size / 2,
    };
          ...
          return <ActionButtonItem
            ...
            onPress={() => {
              if (this.props.autoInactive) {
                this.timeout = setTimeout(this.reset, 300);
              }
  ...
  _renderTappableBackground() {
    return (
      <TouchableOpacity activeOpacity={1} style={this.getOverlayStyles()} onPress={this.reset} />
    );
  }
