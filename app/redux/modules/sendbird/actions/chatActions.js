...
const registerGroupChannelHandler = (channelUrl, dispatch) => {
  ...
  channelHandler.onReactionUpdated = (channel, message) => {
    console.log('message reaction update: ', {message})
    if (message.operation === 'add') {
      dispatch({
        type: ADDED_MESSAGE_REACTION,
        message
      });
    } else {
      dispatch({
        type: REMOVED_MESSAGE_REACTION,
        message 
      })
    }
  }
...
export const getPrevMessagesByTimestamp = (channelUrl, timestamp, options) => {
  return dispatch => {
    sbGetMessages(channelUrl, timestamp, options)
      .then(messages => {
        dispatch({
          type: MESSAGE_LIST_SUCCESS,
          list: messages
...
export const addReaction = ({ channelUrl, message, emoji }) => {
  return dispatch => {
    console.log({ channelUrl, message, emoji })
    sbAddReaction({ channelUrl, message, emoji })
      .then(() => dispatch({ type: ADD_MESSAGE_REACTION }));
  }
}

export const removeReaction = ({ channelUrl, message, emoji }) => {
  return dispatch => {
    sbRemoveReaction({ channelUrl, message, emoji })
      .then(() => dispatch({ type: REMOVE_MESSAGE_REACTION, message }));
  }
}
...
const sendTextMessage = (dispatch, channel, textMessage) => {
  const messageTemp = sbSendTextMessage(channel, textMessage, (message, error) => {
    if (error) {
      ...
      sbCreateExtraData(channel, message, ['reactions', 'reactedUsers'])
...
export const addExtraDataToMessage = (channelUrl, message, emoji) => {
  return dispatch => {
    sbGetGroupChannel(channelUrl)
      .then(channel =>
        sbAddExtraData(channel, message, {
          reactions: ['smile'],
          reactedUsers: ['hello']
        })
          .then(m => dispatch({ type: ADD_MESSAGE_REACTION, message: m }))
          .catch(e => console.log(e))
      )
      .catch(e => console.log(e))
  }
      ...
      .then(channel => {
        return sbChannelUpdateMessage(channel, message, contents)
          .then(response => {
            ...
            dispatch({ type: OWN_MESSAGE_UPDATED, edited: response })
          })
          .catch(error => {
...
export const onSendFileMessage = () => {
  return { type: SEND_FILE_MESSAGE }
}

export const onCancelSendFileMessage = () => {
  return { type: SEND_FILE_MESSAGE_FAIL }
}
