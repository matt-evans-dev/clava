import {
  ...
  JOINED_GROUP_CHANNEL_FAIL
} from './types';
import { sbGetGroupChannelList, sbGetGroupChannel, sbLeaveGroupChannel, sbHideGroupChannel, sbJoinGroupChannel } from '../sendbirdActions';
...
export const onJoinGroupChannel = channelUrl => {
  return dispatch => {
    return sbJoinGroupChannel(channelUrl)
      .then(response =>
        dispatch({
          type: JOINED_GROUP_CHANNEL,
          payload: channelUrl
        })
      )
      .catch(error => dispatch({ type: JOINED_GROUP_CHANNEL_FAIL }));
  };
};
